...
import {
    ChartDot,
    ChartPath, ChartPathProvider, ChartXLabel, ChartYLabel, monotoneCubicInterpolation
...
const Chart = ({ containerStyle, chartPrices }) => {
    ...
    // points
    let startUnixTimesStamp = moment().subtract(7, 'day').unix()
    let data = chartPrices ? chartPrices?.map((item, index) => {
        return {
            x: startUnixTimesStamp + (index + 1) * 3600,
            y: item
        }
    }) : []
    ...
    return (
        <View style={{ ...containerStyle }}>
            ...
            <View style={{
                position: 'absolute',
                left: SIZES.padding,
                top: 0, bottom: 0, justifyContent: 'space-between'
            }}>
                {
                ...
                >
                    ...
                    <ChartDot>
                        <View style={{
                            position: 'absolute',
                            left: -35, width: 80,
                            alignItems: 'center', backgroundColor: COLORS.transparentBlack
                        }}>
                            {/* Dot */}
                            <View style={{
                                alignItems: 'center', justifyContent: 'center',
                                width: 25, height: 25, borderRadius: 15, backgroundColor: COLORS.white
                            }}>
                                <View style={{
                                    width: 15, height: 15,
                                    borderRadius: 10, backgroundColor: COLORS.lightGreen
                                }} />
                            </View>
                            {/* Y-label */}
                            <ChartYLabel
